...
for country_code in unique_countries:
  ...
  TOTAL_STATIONS_IN_COUNTRY = len(unique_stations_in_country)
  ...
  for station_id in unique_stations_in_country:
    ...
    ut.compose_station_console_output(country_iteration, TOTAL_COUNTRIES, country_name, station_iteration, TOTAL_STATIONS_IN_COUNTRY, station_id, anomaly_visual, anomaly_trend, absolute_visual, absolute_trend, start_year, end_year, station_location)
    ...
    # pd.concat([pd.Series([f"{country_name} station averages"]), average_anomolies_of_all_stations])

  ut.print_summary_to_console(TOTAL_STATIONS_IN_COUNTRY, country_code)
  #
  # Output File
  ut.create_excel_file(pd.DataFrame(annual_anomalies_by_station_in_country), country_name, country_code)
